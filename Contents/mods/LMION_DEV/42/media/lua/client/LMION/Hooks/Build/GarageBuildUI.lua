...
ISWidgetInput.updateValues = function(self)
    ...
    local requirement, key = GarageBuildRequirements.getRequirement(
        profile,
        length,
        fullType
    )
    ...
    local available = GarageBuildRequirements.getAvailable(
        self.player,
        key,
        requirement.uses,
...
ISWidgetTitleHeader.updateLabels = function(self)
    ...
    local hasRequirements = GarageBuildRequirements.hasRequirements(
        self.player,
        profile,
        length,
        getContainers(self.logic)
...
ISWidgetBuildControl.prerender = function(self)
    ...
    self.buttonCraft.enable = self.buttonCraft.enable
        and GarageBuildRequirements.hasRequirements(
            self.player,
            profile,
            length,
            getContainers(self.logic)
    ...
    if profile ~= nil and self.buildEntity ~= nil then
        ...
        if not self.player:isBuildCheat() then
            self.buildEntity.blockBuild = self.buildEntity.blockBuild
                or not GarageBuildRequirements.hasRequirements(
                    self.player,
                    profile,
                    length,
                    getContainers(self.logic)
