...
function GarageBuildRequirements.getRequirements(profile, length)
    if profile == nil then
        return nil
    end
...
function GarageBuildRequirements.getStock(character, containers)
    local stock = {}
...
function GarageBuildRequirements.hasRequirements(character, profile, length, containers)
    local requirements = GarageBuildRequirements.getRequirements(profile, length)
    if requirements == nil then
        return true
    end

    local stock = GarageBuildRequirements.getStock(character, containers)
...
function GarageBuildRequirements.getAvailable(character, key, uses, containers)
    local stock = GarageBuildRequirements.getStock(character, containers)
    return getAvailableAmount(stock, key, uses)
end

function GarageBuildRequirements.getRequirement(profile, length, fullType)
...
function GarageBuildRequirements.consumeExtras(
    character,
    profile,
    length,
    containers,
...
function GarageBuildRequirements.recordExtras(buildObject)
...
return GarageBuildRequirements
