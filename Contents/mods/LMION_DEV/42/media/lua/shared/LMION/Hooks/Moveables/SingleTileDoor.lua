...
local function getLargeGateParcelName(segment)
    local profile = segment and segment.profile or nil
    return tostring(profile and profile.displayName or segment.definitionId) .. " " .. tostring(segment.leaf) .. " (" .. tostring(segment.partIndex) .. "/2)"
end

local function configureLargeGateParcel(item, segment, state)
    if item == nil or segment == nil then return end
    local profile = segment.profile
    if profile ~= nil then item:setActualWeight(profile.weight); item:setWeight(profile.weight) end
    item:setName(getLargeGateParcelName(segment)); item:setCustomName(true)
    LargeGateParcel.writeIdentity(item, segment)
    if state ~= nil then LargeGateParcel.writeState(item, state) end
end
...
local function configureGarageParcel(item, segment, state)
    ...
    local profile=segment.profile
    if profile ~= nil then item:setActualWeight(profile.weight); item:setWeight(profile.weight) end
    item:setName(tostring(profile and profile.definition and profile.definition.displayName or segment.definitionId) .. " " .. tostring(segment.role))
    item:setCustomName(true)
    local data=item:getModData(); data.lmionGarageDefinitionId=segment.definitionId; data.lmionGarageRole=segment.role
    if state ~= nil then DoorTransportState.writeToItem(item,state) end
end

local function hasPlacementRequirements(moveProps, character)
    if character == nil or not instanceof(character, "IsoPlayer") then return true end
    if ISMoveableDefinitions.cheat or character:isMovablesCheat() then return true end
...
function SingleTileDoorHook.install()
    if ISMoveableSpriteProps._lmionV3SingleTileDoorInstalled == true then return false end
    ...
    ISMoveableSpriteProps.hasFaces = function(self)
        local garageFaces=GarageMoveProps.getFaces(self); if garageFaces ~= nil then return garageFaces.N ~= garageFaces.W end
        local largeGateFaces = LargeGateMoveProps.getFaces(self); if largeGateFaces ~= nil then return largeGateFaces.N ~= largeGateFaces.W end
        local profile = SingleTileDoorMoveProps.getProfile(self); if profile ~= nil then return profile.faces.N ~= profile.faces.W end
        return originalHasFaces(self)
    end

    ISMoveableSpriteProps.getFaces = function(self)
        local garageFaces=GarageMoveProps.getFaces(self); if garageFaces ~= nil then return garageFaces end
        local largeGateFaces = LargeGateMoveProps.getFaces(self); if largeGateFaces ~= nil then return largeGateFaces end
        local profile = SingleTileDoorMoveProps.getProfile(self)
        if profile ~= nil then return {N=profile.faces.N,W=profile.faces.W} end
    ...
    ISMoveableSpriteProps.pickUpMoveableInternal = function(self, character, square, object, sprInstance, spriteName, createItem, rotating)
        ...
        elseif largeGateSegment ~= nil and DoorObject.isDoor(object) then
            self.lmionPendingLargeGateSegment = largeGateSegment; self.lmionPendingLargeGateState = DoorTransportState.capture(object) or {}
        elseif garageSegment ~= nil and DoorObject.isDoor(object) then
            self.lmionPendingGarageSegment=garageSegment; self.lmionPendingGarageState=DoorTransportState.capture(object) or {}
        end
    ...
    ISMoveableSpriteProps.instanceItem = function(self, spriteNameOverride)
        ...
        if profile ~= nil then spriteName=SingleTileDoorMoveProps.getClosedSpriteName(self,profile,spriteNameOverride)
        elseif largeGateSegment ~= nil then spriteName=getLargeGateClosedSprite(largeGateSegment) or spriteNameOverride
        elseif garageSegment ~= nil then spriteName=getGarageClosedSprite(garageSegment) or spriteNameOverride end
        local item=originalInstanceItem(self,spriteName)
        if profile ~= nil and item ~= nil and self.lmionPendingDoorState ~= nil then DoorTransportState.writeToItem(item,self.lmionPendingDoorState)
        elseif largeGateSegment ~= nil and item ~= nil then configureLargeGateParcel(item,self.lmionPendingLargeGateSegment or largeGateSegment,self.lmionPendingLargeGateState)
        elseif garageSegment ~= nil and item ~= nil then configureGarageParcel(item,self.lmionPendingGarageSegment or garageSegment,self.lmionPendingGarageState) end
        return item
    end

    ISMoveableSpriteProps.canPlaceMoveableInternal = function(self, character, square, item, forceTypeObject)
        local profile = SingleTileDoorMoveProps.getProfile(self)
        if profile == nil then return originalCanPlace(self, character, square, item, forceTypeObject) end
        local facing = SingleTileDoorMoveProps.getFacing(self, profile)
        if not SingleTileDoorPlacement.canPlace(profile, square, facing) then return false end
    ...
    ISMoveableSpriteProps.placeMoveableInternal = function(self, square, item, spriteName)
        local profile = SingleTileDoorMoveProps.getProfile(self)
        if profile == nil then return originalPlace(self, square, item, spriteName) end
        ...
        local finalized = SingleTileDoorPlacementFinalizer.finalize(square, result, item, targetSprite, profile)
